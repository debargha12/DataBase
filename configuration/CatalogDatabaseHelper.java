package chakri.configuration;


import java.io.File;
import java.util.ArrayList;
import java.util.List;
import chakri.model.DataTypeEnum;
import chakri.model.DataType_Int;
import chakri.model.DataType_Text;
import chakri.model.DatabaseConstants;
import chakri.prompt.DataRecord;
import chakri.prompt.IOManager;
import chakri.prompt.InternalColumn;
import chakri.prompt.InternalCondition;
import chakri.prompt.InternalException;
import chakri.prompt.Page;
...
public class CatalogDatabaseHelper {
  ...
  public static void InitializeDatabase() {
    ...
    if (!baseDir.exists()) {
      File catalogDir = new File(DatabaseConstants.DEFAULT_DATA_DIRNAME + "/"
          + DatabaseConstants.DEFAULT_CATALOG_DATABASENAME);
  ...
  public boolean createCatalogDatabase() {
    try {
      ...
      manager.createTable(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
          DatabaseConstants.SYSTEM_TABLES_TABLENAME + DatabaseConstants.DEFAULT_FILE_EXTENSION);
      manager.createTable(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
          DatabaseConstants.SYSTEM_COLUMNS_TABLENAME + DatabaseConstants.DEFAULT_FILE_EXTENSION);
      int startingRowId =
          this.updateSystemTablesTable(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
              DatabaseConstants.SYSTEM_TABLES_TABLENAME, 6);
      startingRowId *= this.updateSystemTablesTable(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
          DatabaseConstants.SYSTEM_COLUMNS_TABLENAME, 8);
      if (startingRowId >= 0) {
        List<InternalColumn> columns = new ArrayList<>();
        columns.add(new InternalColumn("rowid", DataTypeEnum.INT.toString(), false, false));
        columns
            .add(new InternalColumn("database_name", DataTypeEnum.TEXT.toString(), false, false));
        ...
        columns
            .add(new InternalColumn("col_tbl_st_rowid", DataTypeEnum.INT.toString(), false, false));
        columns.add(
            new InternalColumn("nxt_avl_col_tbl_rowid", DataTypeEnum.INT.toString(), false, false));
        this.updateSystemColumnsTable(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
            DatabaseConstants.SYSTEM_TABLES_TABLENAME, 1, columns);
        columns.clear();
        columns.add(new InternalColumn("rowid", DataTypeEnum.INT.toString(), false, false));
        columns
            .add(new InternalColumn("database_name", DataTypeEnum.TEXT.toString(), false, false));
        ...
        columns.add(
            new InternalColumn("ordinal_position", DataTypeEnum.TINYINT.toString(), false, false));
        columns.add(new InternalColumn("is_nullable", DataTypeEnum.TEXT.toString(), false, false));
        this.updateSystemColumnsTable(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
            DatabaseConstants.SYSTEM_COLUMNS_TABLENAME, 7, columns);
      }
      return true;
  ...
  public int updateSystemTablesTable(String databaseName, String tableName, int columnCount) {
    try {
      /*
       * System Tables Table Schema: Column_no Name Data_type 1 rowid INT 2 database_name TEXT 3
       * table_name TEXT 4 record_count INT 5 col_tbl_st_rowid INT 6 nxt_avl_col_tbl_rowid INT
       */
      IOManager manager = new IOManager();
      List<InternalCondition> conditions = new ArrayList<>();
      conditions.add(
          InternalCondition.CreateCondition(CatalogDatabaseHelper.TABLES_TABLE_SCHEMA_TABLE_NAME,
              InternalCondition.EQUALS, new DataType_Text(tableName)));
      conditions.add(
          InternalCondition.CreateCondition(CatalogDatabaseHelper.TABLES_TABLE_SCHEMA_DATABASE_NAME,
              InternalCondition.EQUALS, new DataType_Text(databaseName)));
      List<DataRecord> result = manager.findRecord(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
          DatabaseConstants.SYSTEM_TABLES_TABLENAME, conditions, true);
      if (result != null && result.size() == 0) {
        int returnValue = 1;
        Page<DataRecord> page =
            manager.getLastRecordAndPage(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
                DatabaseConstants.SYSTEM_TABLES_TABLENAME);
        ...
        if (lastRecord == null) {
          ...
          DataType_Int startingColumnIndex = (DataType_Int) lastRecord.getColumnValueList()
              .get(CatalogDatabaseHelper.TABLES_TABLE_SCHEMA_NXT_AVL_COL_TBL_ROWID);
        ...
        record.populateSize();
        manager.writeRecord(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
            DatabaseConstants.SYSTEM_TABLES_TABLENAME, record);
        return returnValue;
      } else {
  ...
  public boolean updateSystemColumnsTable(String databaseName, String tableName, int startingRowId,
    ...
    try {
      /*
       * System Tables Table Schema: Column_no Name Data_type 1 rowid INT 2 database_name TEXT 3
       * table_name TEXT 4 column_name TEXT 5 data_type TEXT 6 column_key TEXT 7 ordinal_position
       * TINYINT 8 is_nullable TEXT
       */
      IOManager manager = new IOManager();
      ...
      for (; i < columns.size(); i++) {
        ...
        record.populateSize();
        if (!manager.writeRecord(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
            DatabaseConstants.SYSTEM_COLUMNS_TABLENAME, record)) {
          break;
        }
      }
      return true;
  ...
  private static int updateRowCount(String databaseName, String tableName, int rowCount) {
    try {
      ...
      return manager.updateRecord(DatabaseConstants.DEFAULT_CATALOG_DATABASENAME,
          DatabaseConstants.SYSTEM_TABLES_TABLENAME, conditions, updateColumnsIndexList,
